...
drawbar(Monitor *m)
{
	...
	for (i = 0; i < LENGTH(tags); i++) {
		//XGlyphInfo extents;
		//XftTextExtentsUtf8(drw->dpy, drw->fonts->xfont, (FcChar8*)tags[i], strlen(tags[i]), &extents);
		//int cw = extents.xOff;
		w = TEXTW(tags[i]);
		int ps = 14; // pixelsize measured beforehand on display
		drw_setscheme(drw, scheme[m->tagset[m->seltags] & 1 << i ? SchemeSel : SchemeNorm]);
		drw_rect(drw, x, 0, w, bh, 1, 1);
		drw_text(drw, x, 0, w, bh, w/2 - ps/2, tags[i], urg & 1 << i);
		//if (occ & 1 << i)
	...
	drw_setscheme(drw, scheme[SchemeNorm]);
        ...
        if (m == selmon) { /* status is only drawn on selected monitor */
                ...
                if (splitstatus) {
                        ...
                        msx = (m->ww - TEXTW(mstext) + lrpad) / 2 - sidepad - 32; /* x position of middle st
atus text */
                        drw_text(drw, msx, 0, TEXTW(mstext) - lrpad, bh, 0, mstext, 0);
                }
                tw = TEXTW(rstext) - lrpad + 2; /* 2px right padding */
                drw_text(drw, m->ww - tw - sidepad - 32, 0, tw, bh, 0, rstext, 0);
        }
